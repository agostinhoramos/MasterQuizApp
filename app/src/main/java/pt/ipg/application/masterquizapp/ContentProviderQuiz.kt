...
class ContentProviderQuiz : ContentProvider() {
    ...
    override fun onCreate(): Boolean {
        bdOpenHelper = QuizDBOpenHelper(context)
        return true
    }
 ...
 * and Threads]({@docRoot}guide/topics/fundamentals/processes-and-threads.html#Threads).
    ...
    override fun getType(uri: Uri): String? {
        return when(uriMatcher().match(uri)) {
            URI_USERS -> "vnd.android.cursor.dir/$USERS"
            URI_USER_ID -> "vnd.android.cursor.item/$USERS"
            else -> null
        }
    }
 ...
 * and Threads](
    ...
    companion object {
        private const val AUTHORITY = "pt.ipg.application.masterquizapp"
        private const val USERS = "users"

        private const val URI_USERS = 200
        private const val URI_USER_ID = 201

        private fun uriMatcher() = UriMatcher(UriMatcher.NO_MATCH).apply {
            addURI(AUTHORITY, USERS, URI_USERS)
            addURI(AUTHORITY, "$USERS/#", URI_USER_ID)
        }

        val ADDRESS: Uri = Uri.parse("content://$AUTHORITY")
        val ADDRESS_USERS = Uri.withAppendedPath(ADDRESS, USERS)
    }
}